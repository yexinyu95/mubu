...
- 多元微分学
			...
			- 曲率
				...
				- 因此曲率$\lim\limits_{\Delta s \to 0} \dfrac{\Delta \alpha}{\Delta s} = \lim\limits_{\Delta s \to 0} \dfrac{||\Delta \vec{\sigma}'||}{\Delta s} = ||\vec{\sigma}''||$，
				  collapsed:: true
					...
					- 因此求导后得到的为二阶导，
				- 对于一般空间曲线，曲率$\kappa(\sigma) = \dfrac{\vec{\sigma}'(t) \times \vec{\sigma}''(t)}{||\vec{\sigma}'(t)^{2}||}$，
	...
	- @多元函数的中值定理
	  collapsed:: true
		...
		- 多元微分中值定理
		  collapsed:: true
			- 设集合D为R^{n}中的凸域，定义在D上的R^{n} \to R的函数 f 存在全微分，
			- 则\forall a, b \in D, \exist \xi in D，
			- 使得$f(\vec{b}) - f(\vec{a}) = Jf(\vec{\xi}) \cdot (\vec{b} - \vec{a})$，其中\xi 位于连接 a, b 的直线段上，
		- 多元拟微分中值定理
		  collapsed:: true
			- 微分中值定理*不能*推广到向量值函数R^{n} \to R^{m}，
				...
				- 偏导数
					- （一阶偏导数）必要条件
					  collapsed:: true
						...
						- 则一定有$\nabla f(x) = (f'_{x}(x_{0}, y_{0}) , f'_{y}(x_{0}, y_{0})) = 0$；
						- 可推广至多元函数，
					- （二阶偏导数）充分条件
					  collapsed:: true
						...
						- 二元情况
						  collapsed:: true
							...
							- Hessian矩阵为 $\begin{pmatrix} \dfrac{\partial^2 f} {\partial x^2} & \dfrac{\partial^2 f} {\partial x \partial y} \\\dfrac{\partial^2 f} {\partial y \partial x} & \dfrac{\partial^2 f} {\partial y^2} \end{pmatrix}$ = $\begin{pmatrix} A & B \\B & C \end{pmatrix}$ ，
							- 由判断矩阵正定的赫尔维兹定理，
							- 若所有主子式（A，AC-B^{2}）均大于0，则矩阵正定，即该点为极小值，
							- 若奇数阶主子式（A）均小于0，偶数阶（AC-B^{2}）主子式均大于0，则矩阵负定，即该点为极大值，
							- 若偶数阶主子式（AC-B^{2}）小于0，则矩阵不定，该点不是极值点，
						- （证明：Taylor展开式）
			- 极值的求解技巧
			  collapsed:: true
				- （分析区域：函数定义域、区域、边界等）
		  ...
		  collapsed:: true
			...
			- 二重积分换元
			  collapsed:: true
				...
				- 其它变量换元
					- 应注意积分区域、积分函数（f_{xy} (u,v) \cdot |J|）都要改变，
			- 二重积分分部积分
			  collapsed:: true
