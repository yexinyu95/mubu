	...
	- @多元函数的中值定理
		...
		- 多元微分中值定理
		  collapsed:: true
			...
			- 证明（多元函数转换为一元函数）
			  collapsed:: true
				- 对于多元函数的问题，应尝试控制变量或构造变量，来将其转化为一元函数，并应用一元函数的方式求解，
			...
			- 极值的判断
				...
				- 偏导数
					...
					- （二阶偏导数）充分条件
						...
						- 若x_{0}为驻点（\nabla f(x_{0}) = 0），则Hessian*半正定/负定*时，函数有极值，
						  collapsed:: true
							...
							- Hessian矩阵不定，该点不是极值点（称为鞍点），
							  collapsed:: true
								- “鞍点”来自于马鞍面（双曲抛物面），
								- ![image.png](../assets/image_1664331543644_0.png){:height 177, :width 261}，
						...
						- 证明：Taylor展开式
						  collapsed:: true
							- $f(x, y) \approx f(x_0, y_0) + f'_x|_{(x_{0}, y_{0})}(x - x_{0}) + f'_y|_{(x_{0}, y_{0})}(y - y_{0}) + \frac{1}{2}[f''_{xx}|_{(x_{0}, y_{0})}(x - x_{0})^{2} + 2f''_{xy}|_{(x_{0}, y_{0})}(x - x_{0})(y - y_{0}) + f''_{yy}|_{(x_{0}, y_{0})}(y - y_{0})^{2}]$，
			- 极值的求解技巧
			  collapsed:: true
				- （分析区域：函数定义域、区域、边界等）
				- 函数的分析
